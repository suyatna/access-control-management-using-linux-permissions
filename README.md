...
Proyek ini menggambarkan penerapan kontrol akses berbasis sistem operasi melalui pengelolaan izin file dan direktori di lingkungan Linux. Skenario disusun berdasarkan kondisi nyata, di mana data milik tim riset perlu dijaga agar hanya dapat diakses oleh pihak yang berwenang. Fokus utama diarahkan pada penerapan prinsip least privilege agar setiap pengguna hanya memiliki hak akses yang benar-benar dibutuhkan.

Proses dimulai dengan meninjau seluruh file dan direktori menggunakan perintah seperti `ls -la` untuk memahami struktur izin yang sedang berjalan. Setiap permission dianalisis secara detail, termasuk hak baca, tulis, dan eksekusi pada level owner, group, dan other. Tahap ini membantu mengidentifikasi konfigurasi yang terlalu longgar dan berpotensi menimbulkan risiko keamanan.

Penyesuaian izin dilakukan secara bertahap menggunakan perintah `chmod` agar akses yang tidak diperlukan dapat dihapus tanpa mengganggu pekerjaan pengguna. File tersembunyi juga ikut diperiksa karena meskipun tidak terlihat secara langsung, file jenis ini tetap menyimpan risiko yang sama jika izinnya tidak dikonfigurasi dengan benar. Direktori dengan tingkat sensitivitas tinggi mendapat perlakuan khusus agar hanya pemilik yang dapat masuk dan mengakses isinya.

Hasil akhir menunjukkan bahwa pengelolaan izin yang tepat mampu meningkatkan keamanan sistem secara signifikan sekaligus menjaga stabilitas operasional. Alur kerja pengguna tetap berjalan normal, sementara risiko perubahan data tanpa otorisasi dapat ditekan. Proyek ini mencerminkan pemahaman praktis tentang access control, Linux permissions, dan peran penting konfigurasi sistem sebagai lapisan dasar dalam menjaga keamanan organisasi secara menyeluruh.
